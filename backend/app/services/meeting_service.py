...
# ✅ CREATE MEETING (NOW ACCEPTS user_id)
def create_meeting(data: dict, user_id: str):
    meeting = {
    ...
    # Store embeddings if notes exist
    if meeting.get("notes"):
        chunks = split_text(meeting["notes"])
        store_chunks(chunks, meeting_id)
...
# ✅ PROCESS MEETING (SECURED BY USER)
def process_meeting(meeting_id: str, user_id: str):
    meeting = meetings_collection.find_one({
        "_id": ObjectId(meeting_id),
        "user_id": user_id   # ✅ SECURITY FIX
    })

    if not meeting:
        return None

    # Store embeddings again if needed
    if meeting.get("notes"):
        chunks = split_text(meeting["notes"])
        store_chunks(chunks, meeting_id)
