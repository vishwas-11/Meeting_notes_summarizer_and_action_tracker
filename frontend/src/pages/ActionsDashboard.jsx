...
export default function ActionsDashboard() {
  ...
  const inputStyle = "bg-white/40 backdrop-blur-sm border border-[#EAE7DF] rounded-xl px-4 py-2 text-sm text-[#4A4A4A] focus:outline-none focus:ring-2 focus:ring-[#6B705C]/10 focus:border-[#6B705C] transition-all placeholder:text-[#A3A199]";
    ...
    <div className="min-h-screen pt-24 pb-12 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          ...
          <div className="bg-white/40 backdrop-blur-md border border-white p-4 rounded-2xl shadow-sm min-w-[240px]">
            <div className="flex justify-between items-end mb-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#8C7E6A]">Completion</span>
              <span className="text-sm font-medium text-[#6B705C]">{Math.round(percentage)}%</span>
            </div>
            <div className="w-full bg-[#EFEBE3]/60 h-1.5 rounded-full overflow-hidden">
              <div
                className="bg-[#6B705C] h-full rounded-full transition-all duration-700 ease-out"
                style={{ width: `${percentage}%` }}
              />
        ...
        <div className="flex flex-wrap gap-4 mb-8 p-2 bg-[#F2EFE9]/40 backdrop-blur-sm rounded-2xl border border-white/20">
          <div className="relative flex-1 min-w-[200px]">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#A3A199]" size={16} />
            <input
              placeholder="Search tasks..."
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 bg-white/10 backdrop-blur-sm border border-dashed border-[#EAE7DF] rounded-3xl">
            <CheckCircle className="mx-auto text-[#EAE7DF] mb-4" size={48} />
            <p className="text-[#8C7E6A] italic">No actions found matching your filters.</p>
          </div>
        ) : (
